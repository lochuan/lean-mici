#!/usr/bin/env bash
# harvest_device_prebuilt.sh — 从设备回收原生构建的 camerad/loggerd（mac 上跑）。
#
# 容器构建不出/错这两份二进制：
...
FILES="openpilot/system/camerad/camerad openpilot/system/loggerd/loggerd"
...
for f in $FILES; do
  ssh -o BatchMode=yes "$DEVICE" "test -x /data/openpilot/$f" || {
