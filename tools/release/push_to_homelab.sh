...
for img in "$HOME/Documents/Projects/agnos-builder/output"/system-*.img.xz; do
  [ -f "$img" ] || continue
  name="$(basename "$img")"
  remote_sha="$(ssh "$HOMELAB" "sha256sum ~/agnos-ota/www/$name 2>/dev/null" | cut -d" " -f1 || true)"
  local_sha="$(sha256sum "$img" | cut -d" " -f1)"
  if [ "$remote_sha" = "$local_sha" ]; then
    echo "skip $name (identical)"
  else
    echo "upload $name"
    scp "$img" "$HOMELAB:~/agnos-ota/www/"
    ssh "$HOMELAB" "cd ~/agnos-ota/www && sha256sum $name | tee $name.sha256"
  fi
done
echo done
