...
from openpilot.selfdrive.avoidanced.constants import CAMERA_PITCH, CAMERA_TO_FRONT, CAMERA_YAW, VEHICLE_WEIGHT, VRU_CLASSES, VRU_WEIGHT
...
class RoiMeta:
  ...
  offset_v: float  # full-frame row of the ROI top edge (bottom crop)


def roi_meta_for(width: float, height: float) -> RoiMeta:
  """ROI geometry for a full frame: bottom crop at the YOLO aspect, then resize."""
  crop_h = min(height, round(width * INPUT_H / INPUT_W))
  return RoiMeta(scale_u=width / INPUT_W, scale_v=crop_h / INPUT_H, offset_v=height - crop_h)


def roi_to_full(u: float, v: float, meta: RoiMeta) -> tuple[float, float]:
  """ROI pixel -> original full-frame pixel."""
  return u * meta.scale_u, v * meta.scale_v + meta.offset_v
