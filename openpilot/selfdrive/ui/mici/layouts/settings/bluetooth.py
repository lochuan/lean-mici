...
from openpilot.sunnypilot.system.bluetooth.manager import BluetoothManager
...
class BluetoothDeviceButton(BigButton):
  ...
  def _draw_content(self, btn_y: float):
    ...
    label_rect = rl.Rectangle(
      self._rect.x + self.LABEL_PADDING, btn_y + self.LABEL_VERTICAL_PADDING, self.LABEL_WIDTH, self._rect.height - self.LABEL_VERTICAL_PADDING * 2
    )
    ...
    if self.device.connected:
      ...
      rl.draw_texture_ex(self._check_txt, rl.Vector2(sub_label_x, check_y), 0.0, 1.0, rl.Color(255, 255, 255, int(255 * 0.585)))
...
class BluetoothScanningButton(BigButton):
  ...
  def _draw_content(self, btn_y: float):
    ...
    animation.set_position(self._rect.x + self._rect.width - animation.rect.width - 40, btn_y + self._rect.height - animation.rect.height - 30)
    animation.render()
