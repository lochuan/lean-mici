...
def _wait_held(core, worker, v_ego=20.0, now=0.2, timeout=2.0):
  ...
  for _ in range(int(timeout / 0.01)):
    frame = core.process(_core_sm(), now, v_ego, C.CAMERA_TO_FRONT, vision_enabled=True, vision_due=False)
    if frame.detections:
      return frame
    time.sleep(0.01)
  return frame
...
def test_late_worker_result_discarded_after_disable():
  ...
  try:
    ...
    core.process(_core_sm(), 0.1, 20.0, C.CAMERA_TO_FRONT, vision_enabled=False, vision_due=False)  # 清 hold + 代际
    gate.set()
    finished.wait(2.0)
    time.sleep(0.05)                             # 让结果落入 worker 邮箱
    frame = core.process(_core_sm(), 0.2, 20.0, C.CAMERA_TO_FRONT, vision_enabled=True, vision_due=False)
