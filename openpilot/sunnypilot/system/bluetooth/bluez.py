...
from openpilot.sunnypilot.system.bluetooth.protocol import device_capabilities, show_pairing_device
...
AGENT_PATH = "/link/sunnypilot/agent"
...
class BlueZClient:
  ...
  def devices(self, objects: dict[str, Any] | None = None, include_hidden: bool = False, include_discovering: bool = False) -> list[dict[str, Any]]:
    objects = self.managed_objects() if objects is None else objects
    devices = []
    for path, interfaces in objects.items():
      ...
      if include_hidden or show_pairing_device(
        device["address"], device["name"], device["paired"], device["trusted"], device["connected"], device["blocked"], audio, controller, include_discovering
      ):
