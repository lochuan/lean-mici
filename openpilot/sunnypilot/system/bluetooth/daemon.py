...
from openpilot.sunnypilot.system.bluetooth.bluez import BlueZClient
from openpilot.sunnypilot.system.bluetooth.protocol import BLUETOOTH_SOCKET_PATH
from openpilot.sunnypilot.system.bluetooth.radio import BluetoothRadio


OFFROAD_COMMANDS = {"set_power", "start_scan", "stop_scan", "pair", "forget", "test_audio", "pairing_response"}
MEMORY_PARAMS_PATH = os.path.join(Paths.shm_path(), "params")
SCAN_DURATION = 20.0
...
class BluetoothController:
  def __init__(
    self, params: Params | None = None, bluez_factory=BlueZClient, radio: BluetoothRadio | None = None, params_memory: Params | None = None, sleep=time.sleep
  ):
    self.params = params or Params()
    self.params_memory = params_memory or Params(MEMORY_PARAMS_PATH)
  ...
  def status(self) -> dict[str, Any]:
    ...
    with self._lock:
      result = {
        ...
        "selected_audio": self.params.get("BluetoothAudioAddress") or "",
        "devices": [],
        "prompt": None,
  ...
  def _pair_worker(self, address: str) -> None:
    try:
      ...
      if status.get("audio") and not self.params.get("BluetoothAudioAddress"):
        self.params.put("BluetoothAudioAddress", address, block=True)
      self._pairing_error = ""
    except Exception as error:
  ...
  def _test_audio_worker(self, address: str, deadline: float) -> None:
    try:
      ...
      if not self._offroad() or not self.params.get_bool("BluetoothEnabled") or (self.params.get("BluetoothAudioAddress") or "").upper() != address.upper():
        return
      device = self._client().device_for_address(address)
      if not device.get("connected"):
        return
      self.params_memory.put("TestAlert", "engage", block=True)
  ...
  def handle(self, request: dict[str, Any]) -> dict[str, Any]:
    ...
    if command == "set_power":
      ...
      with self._lock:
        if enabled:
          try:
            self.params.put_bool("BluetoothEnabled", True, block=True)
            self._client()
          except Exception:
            self.params.put_bool("BluetoothEnabled", False, block=True)
            self._reset_client()
            try:
              self._radio.stop()
            except Exception:
      ...
      if (self.params.get("BluetoothAudioAddress") or "").upper() == address.upper():
      ...
      self.params.put("BluetoothAudioAddress", address, block=True)
      self.params.put_bool("BluetoothAudioTestActive", True, block=True)
  ...
  def _maintain_reconnects(self, status: dict[str, Any], now: float, suspend_controller_reconnect: bool) -> None:
    ...
    controller_candidates = {
      device["address"].upper() for device in candidates if device["controller"] or device["address"].upper() in self._policy_disconnected
    }
    ...
    for device in candidates:
      ...
      try:
        ...
        attempts += 1
        delay = CONTROLLER_RECONNECT_INTERVAL_SECONDS if controller else min(RECONNECT_INTERVAL_SECONDS * (2 ** (attempts - 1)), RECONNECT_MAX_BACKOFF_SECONDS)
