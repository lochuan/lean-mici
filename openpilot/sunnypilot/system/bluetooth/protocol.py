...
BLUETOOTH_SOCKET_PATH = "/tmp/sunnypilot-bluetooth.sock"
...
def show_pairing_device(
  address: str, name: str, paired: bool, trusted: bool, connected: bool, blocked: bool, audio: bool, controller: bool, discovering: bool = False
) -> bool:
...
class BluetoothClient:
  ...
  def _get_desktop_fake(self) -> _DesktopFakeBluetooth | None:
    if not (
      os.getenv("SP_ALLOW_DESKTOP_FAKE_BLUETOOTH", "0").lower() in TRUE_VALUES
      and os.getenv("SIMULATION", "0").lower() in TRUE_VALUES
      and os.getenv("NOBOARD", "0").lower() in TRUE_VALUES
  ...
  def status(self) -> BluetoothStatus:
    ...
    if not os.path.exists(self.socket_path):
      ...
      return BluetoothStatus(
        ...
        selected_audio=params.get("BluetoothAudioAddress") or "",
      )
  ...
  def set_power(self, enabled: bool) -> None:
    ...
    if bootstrap:
      params.put_bool("BluetoothEnabled", True, block=True)
      deadline = time.monotonic() + max(self.timeout, 45.0)
      while not os.path.exists(self.socket_path):
        if time.monotonic() >= deadline:
          params.put_bool("BluetoothEnabled", False, block=True)
