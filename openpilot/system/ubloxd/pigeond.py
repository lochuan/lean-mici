...
def split_assistnow_messages(dat: bytes) -> list[bytes]:
  """AssistNow payload(背靠背 UBX 帧) -> 帧列表;非 UBX 开头直接断言拒绝。"""
  msgs = []
  while len(dat) > 0:
...
def get_assistnow_messages() -> list[bytes]:
  # lean 边界:comma 云端 AGPS 代理(common/api)已随 lean trim 删除,只保留
  # u-blox AssistNow 直连(自备 token)。没有 token 就抛 RuntimeError——
  # 调用方(run_receiving 的辅助线程)吞掉并降级冷启动,GNSS 不依赖云端。
  params = Params()
  token = params.get('AssistNowToken')
  if not token:
    raise RuntimeError("AssistNowToken not configured")
  ...
  r.raise_for_status()
  return split_assistnow_messages(r.content)
